 ...
 * 难以排查的 ENOENT。
 */
function locateHelperScript(): string {
  ...
  if (!found) {
    throw new Error(
      `[text-translation] 未找到 python/local_helper.py（已检查: ${candidates.join(', ')}）。` +
...
function describeCommand(pythonBin: string, helperScript: string, args: string[]): string {
  return [pythonBin, helperScript, ...args].join(' ')
}

function formatTail(buf: string | Buffer | undefined, max = 2000): string {
  if (!buf) return ''
  const text = Buffer.isBuffer(buf) ? buf.toString('utf-8') : buf
  return text.length > max ? `…${text.slice(-max)}` : text
}

function ensureFileExists(p: string, argName: string): void {
  if (!fs.existsSync(p)) {
  ...
  try {
    try {
      ...
      if (e?.name === 'AbortError') {
        throw new Error(
          `[text-translation] 命令执行超时(${Math.round(timeoutMs / 1000)} 秒)，已终止子进程: ${command}`,
        )
      }
      if (e?.code === 'ENOENT') {
        throw new Error(
          `[text-translation] 无法执行命令(ENOENT)，请检查 pythonPath 配置或 Python 是否已安装: ${command}`,
        )
      }
      if (e?.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER') {
        throw new Error(
          `[text-translation] 命令输出超过 ${MAX_BUFFER / 1024 / 1024}MB 上限，请为 extract_text 传入 output_json 将结果写入文件: ${command}`,
        )
      }
      throw new Error(
        `[text-translation] 命令执行失败(退出码 ${e?.code ?? '未知'}): ${command}\n${formatTail(e?.stderr) || formatTail(e?.stdout)}`,
      )
    }
    ...
    try {
      ...
      throw new Error(
        `[text-translation] ${path.basename(helperScript)} 输出不是有效 JSON，前 500 字符:\n${stdout.slice(0, 500)}`,
      )
    }
    if (typeof data !== 'object' || data === null || Array.isArray(data)) {
      throw new Error(
        `[text-translation] ${path.basename(helperScript)} 返回了意外的 JSON 结构: ${JSON.stringify(data).slice(0, 500)}`,
      )
    }
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.tools.register(
    defineTool({
      name: 'extract_text',
      description:
        '从游戏脚本或长文档(Mtool JSON, XLSX, CSV, RenPy, 字幕 SRT/VTT/ASS, PO, PDF, Word, Markdown, TXT)提取可翻译文本：对控制符与变量(\\c[1], {0}, <color>, [ruby])做防爆遮罩并按行数切片。' +
  ...
  ctx.tools.register(
    defineTool({
      name: 'assemble_text',
      description:
        '把翻译审校完成的文本还原控制符与变量，100% 无损组装写回原始格式(JSON/XLSX/CSV/RPY/SRT/VTT/ASS/PO/PDF/DOCX/MD/TXT)，并报告已装配条目数。',
      parameters: {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        '快速检查文本文件的格式、文本条目数、说话人列表及变量标签分布（轻量模式，不输出全文，适合翻译前摸底）。',
      parameters: {
